...
void touch(Adafruit_TFTLCD &tft, uint16_t x_0, uint16_t x_1, uint16_t y_0, uint16_t y_1) {
    /*  init_adc() or select_adc_14/15() affect adc inputs to touch screen, 
        must reset ADCSRA to adrduino defaults 
        (arduino default registers values are from simple serial prints) 
    */
    ADCSRA = 0b10000111;
...
void touch(Adafruit_TFTLCD &tft, uint16_t x_0, uint16_t x_1, uint16_t y_0, uint16_t y_1, uint32_t timeout) {
    /*  init_adc() or select_adc_14/15() affect adc inputs to touch screen, 
        must reset ADCSRA to adrduino defaults 
        (arduino default registers values are from simple serial prints) 
    */
